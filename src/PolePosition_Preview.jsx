...
const CARS_SEED = [
  ...
  {id:5,make:"Tata",model:"Nexon EV",year:2022,fuel:"Electric",transmission:"Automatic",km:18000,seats:5,price:1400000,score:93,badge:"Hot",img:"https://images.unsplash.com/photo-1593941707882-a5bba14938c7?auto=format&fit=crop&w=800&q=80",images:["https://images.unsplash.com/photo-1593941707882-a5bba14938c7?auto=format&fit=crop&w=800&q=80","https://images.unsplash.com/photo-1593941707874-ef25b8b4a92b?auto=format&fit=crop&w=800&q=80","https://images.unsplash.com/photo-1571127236794-81c0bbfe1ce3?auto=format&fit=crop&w=800&q=80"],category:"EV SUV",tagline:"The future, right now.",owners:1,insurance:"Valid till Feb 2026",scoreBreakdown:{Engine:95,Body:93,Interior:91,Electrical:96,Tyres:90,Docs:94},specs:{_raw:{engine_detail:"Permanent Magnet Synchronous Motor",key_engine:"—",max_power:"127 bhp",max_torque:"245 Nm",driving_range:"312 km",top_speed:"120",acceleration_0_100:"9.9",drivetrain:"FWD",key_transmission:"Automatic",key_fuel_type:"Electric",battery:"30.2 kWh",length_mm:"3993",width_mm:"1811",height_mm:"1606",wheelbase_mm:"2498",ground_clearance:"205",kerb_weight:"1400 kg",doors:"5",key_seating_capacity:"5",bootspace:"350",front_suspension:"Independent McPherson Strut",rear_suspension:"Twist Beam",front_brake_type:"Disc",rear_brake_type:"Drum",steering_type:"Electric Power Steering",front_tyres:"215/60 R16",rear_tyres:"215/60 R16",spare_wheel:"Steel",airbags:"2",abs:"Yes",ebd:"Yes",brake_assist:"Yes",esp:"Yes",ncap_rating:"5 Star",tpms:"Yes",hill_hold_control:"Yes",traction_control:"Yes",child_seat_anchors:"Yes",seatbelt_warning:"Yes",engine_immobilizer:"Yes",central_locking:"Yes",child_safety_lock:"Yes",air_conditioner:"Yes",front_ac:"Yes",keyless_start:"Yes",cruise_control:"Yes",parking_sensors:"Rear",parking_assist:"Reverse Camera",steering_adjustment:"Tilt",power_outlets_12v:"Yes",seat_upholstery:"Leatherette",leather_steering_wheel:"Yes",driver_armrest:"Yes",rear_armrest:"Yes",folding_rear_seat:"Yes",driver_seat_adjustment:"Manual",power_windows:"Yes",one_touch_up:"Driver",one_touch_down:"Driver",adjustable_orvm:"Electric",turn_indicators_orvm:"Yes",rear_defogger:"Yes",rear_wiper:"Yes",sunroof:"Electric",fog_lights:"Yes",daytime_running_lights:"Yes",headlights:"Projector",auto_headlamps:"Yes",follow_me_home:"Yes",roof_antenna:"Shark Fin",instrument_cluster:"Digital",trip_meter:"Yes",distance_to_empty:"Yes",low_fuel_warning:"Yes",smart_connectivity:"Apple CarPlay & Android Auto",music_system:"Yes",display:"7-inch Touchscreen",head_unit_size:"7 inch",gps_navigation:"Yes",speakers:"8 Speakers (Harman)",usb:"Yes",aux:"Yes",bluetooth:"Yes",am_fm_radio:"Yes",steering_controls:"Yes",voice_command:"Yes",emission_standard:"Zero Emission"}}},
...
// ── HomePage ─────────────────────────────────────────────────────
function HomePage({setPage,setSelectedCar,setSelectedPost,favs,toggleFav,cars,blog,testimonials,compare,toggleCompare}){
        ...
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          ...
          <tbody>{cars.map(c=><tr key={c.id} style={{borderBottom:"1px solid var(--pp-border)"}}>
            ...
            <td style={{padding:"11px 16px"}}><div style={{display:"flex",gap:6}}><Btn small onClick={()=>setEdit(c)}><Edit2 size={11}/> Edit</Btn><Btn small danger onClick={async()=>{
              if(!window.confirm(`Delete ${c.make} ${c.model} ${c.year}? This can't be undone.`))return;
              const {error}=await supabase.from("cars").delete().eq("id",c.id);
              if(error){alert("Delete failed: "+error.message);return;}
              setCars(cars.filter(x=>x.id!==c.id));
            }}><Trash2 size={11}/> Del</Btn></div></td>
          </tr>)}</tbody>
        </table>
      </Card>
    </div>
